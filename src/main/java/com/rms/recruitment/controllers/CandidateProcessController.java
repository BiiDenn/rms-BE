...
public class CandidateProcessController {
    ...
    @Operation(summary = "Get candidate's recruitment processes", description = "Retrieve all recruitment processes linked to a candidate")
    public ResponseEntity<List<RecruitmentProcessResponse>> getRecruitmentProcessesByCandidateId(
            @PathVariable Integer candidateId) {
        try {
            List<RecruitmentProcessResponse> processes = recruitmentProcessService
                    .getRecruitmentProcessesByCandidateId(candidateId);
    ...
    @Operation(summary = "Create candidate recruitment process", description = "Create and link a new recruitment process for the candidate")
    public ResponseEntity<RecruitmentProcessResponse> createRecruitmentProcess(
        ...
        try {
            RecruitmentProcessResponse response = recruitmentProcessService.createRecruitmentProcess(candidateId,
                    request);
    ...
    @Operation(summary = "Get candidate timeline for a recruitment process", description = "Return all candidate processes for the given candidate and recruitment process")
    public ResponseEntity<List<CandidateProcessTimelineItemResponse>> getCandidateProcessTimeline(
        ...
        List<CandidateProcessTimelineItemResponse> timeline = recruitmentProcessService
                .getCandidateProcessTimeline(candidateId, recruitProcessId);
        return ResponseEntity.ok(timeline);
    }

    @GetMapping("/{candidateId}/recruitment-processes/{recruitProcessId}/interview-evaluation/overview")
    @Operation(summary = "Get interview overview", description = "Return candidate info, interview session info, and interviewer info")
    public ResponseEntity<InterviewOverviewResponse> getInterviewOverview(
            @PathVariable Integer candidateId,
            @PathVariable Integer recruitProcessId) {
        InterviewOverviewResponse overview = recruitmentProcessService.getInterviewOverview(candidateId,
                recruitProcessId);
